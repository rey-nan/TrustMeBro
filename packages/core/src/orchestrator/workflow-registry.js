...
export class WorkflowRunStore {
    ...
    save(run) {
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
    `);
        stmt.run(run.id, run.workflowId, run.status, run.input, JSON.stringify(run.stepResults), run.finalOutput ?? null, run.error ?? null, run.startedAt, run.completedAt ?? null, run.totalTokens, run.startedAt);
    }
    get(id) {
    ...
    listByWorkflow(workflowId, limit = 20) {
        const rows = this.db.prepare('SELECT * FROM workflow_runs WHERE workflow_id = ? ORDER BY started_at DESC LIMIT ?').all(workflowId, limit);
        return rows.map((row) => ({
            id: row.id,
            workflowId: row.workflow_id,
            status: row.status,
    ...
    listRecent(limit = 50) {
        const rows = this.db.prepare('SELECT * FROM workflow_runs ORDER BY started_at DESC LIMIT ?').all(limit);
        return rows.map((row) => ({
            id: row.id,
            workflowId: row.workflow_id,
            status: row.status,
