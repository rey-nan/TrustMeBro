...
import { AgentConfig, AgentTask, AgentResult, AgentStatus } from './types.js';
import { ContextManager } from './context.js';
import type { AgentCommunication, Message, Mention } from './communication.js';
...
export class AgentRunner {
  ...
  async run(config: AgentConfig, task: AgentTask): Promise<AgentResult> {
        ...
        if (this.communication) {
          ...
          for (const mentionedId of mentions) {
            this.communication.send(
              task.agentId,
              mentionedId,
              `You were mentioned in task ${task.id}: ${finalContent.slice(0, 500)}`
            );
          }
        }
  ...
  private async executeToolCallsWithLLM(
    config: AgentConfig,
    task: AgentTask,
  ...
  private async executeWithTimeout(
    ...
    try {
      ...
      if (this.communication) {
        ...
        if (inbox.length > 0) {
          const summarized = inbox.slice(0, 5).map((m) =>
            `From @${m.fromAgentId}: ${m.content.slice(0, 200)}`
          ).join('\n\n');
        ...
        if (mentions.length > 0) {
          const summarizedMentions = mentions.slice(0, 5).map((m) =>
      ...
      const systemMessage: LLMMessage = { role: 'system', content: config.systemPrompt };
