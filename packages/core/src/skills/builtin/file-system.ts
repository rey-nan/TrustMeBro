...
async function readFileImpl(path: string, workspacePath: string): Promise<ToolOutput> {
...
async function writeFileImpl(path: string, content: string, workspacePath: string): Promise<ToolOutput> {
  if (isPathTraversal(path)) {
    return { success: false, error: 'Path traversal not allowed' };
  }
...
async function listFilesImpl(path: string | undefined, workspacePath: string): Promise<ToolOutput> {
...
async function deleteFileImpl(path: string, workspacePath: string): Promise<ToolOutput> {
  if (isPathTraversal(path)) {
    return { success: false, error: 'Path traversal not allowed' };
  }
...
export function createFileSystemSkill(workspacePath: string): Skill {
