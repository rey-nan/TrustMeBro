...
function ensureMemoryFile(workspacePath) {
    const memoryPath = getMemoryPath(workspacePath);
    if (!existsSync(memoryPath)) {
        writeFileSync(memoryPath, `# Agent Memory\n\n`, 'utf-8');
    }
}
async function rememberImpl(key, value, workspacePath) {
    try {
...
async function recallImpl(key, workspacePath) {
    try {
        const memoryPath = getMemoryPath(workspacePath);
        if (!existsSync(memoryPath)) {
...
async function updateWorkingImpl(content, workspacePath) {
    try {
...
async function readWorkingImpl(workspacePath) {
    try {
        const workingPath = join(workspacePath, WORKING_FILE);
...
export function createMemoryToolsSkill(workspacePath) {
    const rememberTool = {
        definition: {
            name: 'remember',
