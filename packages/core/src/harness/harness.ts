...
import { ReasoningBudget, type ReasoningConfig } from './reasoning-budget.js';
...
export class Harness {
  ...
  private verification: SelfVerification;
  private loopDetector: LoopDetector;
  private traceAnalyzer: TraceAnalyzer;
  private ralph: RalphLoop;
  private pipeline: MiddlewarePipeline;
  private loopProtection: LoopProtectionMiddleware;
  ...
  async execute(originalConfig: AgentConfig, task: AgentTask): Promise<AgentResult> {
    this.logger.info({
    ...
    while (attempts < maxAttempts) {
      ...
      try {
        ...
        const result = await this.ralph.execute<AgentResult>(
          () => this.runner.run(enhancedConfig, currentTask),
          {
            maxRetries: 1,
  ...
  private async addSuccessToKnowledge(agentId: string, task: string, output: string): Promise<void> {
  ...
  private async addErrorToKnowledge(agentId: string, task: string, error: string): Promise<void> {
    if (!this.knowledgeBase) return;
  ...
  private getLevel(level: import('./reasoning-budget.js').ReasoningLevel): string {
    return level;
  }
  ...
  ): { level: import('./reasoning-budget.js').ReasoningLevel; temperature: number; maxTokens: number } {
