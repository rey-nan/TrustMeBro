...
export class SelfVerification {
  ...
  constructor(client: LLMClient, verificationThreshold: number = 60, logger?: pino.Logger) {
    this.client = client;
    this.verificationThreshold = verificationThreshold;
    this.logger = logger ?? pino({ name: 'SelfVerification' });
  }

  async verify(task: AgentTask, result: AgentResult): Promise<VerificationResult> {
...
- 0-49: Task failed or irrelevant response`;
  ...
  async verifyPlan(plan: string): Promise<VerificationResult> {
...
- 0-49: Plan is incomplete, unclear, or not achievable`;
  ...
  private parseResponse(content: string): Omit<VerificationResult, 'passed'> {
    try {
      const jsonMatch = content.match(/\{[\s\S]*\}/);
      if (jsonMatch) {
        const parsed = JSON.parse(jsonMatch[0] as string) as {
          score: number;
          reasons: string[];
          suggestion?: string;
        };
