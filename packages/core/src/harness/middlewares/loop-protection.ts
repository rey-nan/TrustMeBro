...
export class LoopProtectionMiddleware implements Middleware {
  name = 'loop-protection';
  phase: 'post-execution' = 'post-execution';
  private maxEditsPerResource: number;
  private editCounts: Map<string, Map<string, EditCount>>;
  private logger: pino.Logger;

  constructor(maxEditsPerResource: number = 5, logger?: pino.Logger) {
  ...
  async execute(ctx: MiddlewareContext): Promise<MiddlewareResult> {
    ...
    for (const file of mentionedFiles) {
      ...
      if (current.count > this.maxEditsPerResource) {
        warnings.push(
          `You have modified "${file}" ${current.count} times. Consider reconsidering your approach entirely. Try a different strategy.`
        );
      }
    }
